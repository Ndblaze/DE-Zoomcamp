...
# Specify the months for 2019
months = [f"2019-{str(month).zfill(2)}" for month in range(1, 13)]  # List for 2019 months
...
# Process each month for 2019
for month in months:
    file_url = f"https://github.com/DataTalksClub/nyc-tlc-data/releases/download/fhv/fhv_tripdata_{month}.csv.gz"
    gz_file = f"fhv_tripdata_{month}.csv.gz"
    csv_file = f"fhv_tripdata_{month}.csv"  # Unzipped file will be saved as .csv
    gcs_path = f"fhv_tripdata/{csv_file}"  # Organizing in a subfolder in GCS
